...
(** [surrounding cell] are simply the bottom 3, top 3, and left and right
    neighbors of cell without regards to the bounds *)
...
(** [in_bounds cell ~height ~width] if and only if cell.x >= 0 && cell.x <
    width, and cell.y >= 0 && cell.y < height *)
let in_bounds ({ x; y } : t) ~(width : int) ~(height : int) =
  x >= 0 && x < width && y >= 0 && y < height

(** [neighbors cell ~width ~height] is the set of all cells that are neighbors
    of the given [cell] and satisfies [in_bounds cell ~width ~height]. *)
...
(** [alive_neighbors cell grid] is the set of coordinates that are currently
    within +-1 in the x and or y direction of [cell] and is in [grid] *)
let alive_neighbors (coordinate : t) ({ coordinates; width; height } : grid) :
    CoordinateSet.t =
  neighbors coordinate ~width ~height |> Set.inter coordinates

(** [survive_set curr] is the set of all cells in the current grid that have
    survived *)
let survive_set ({ coordinates; width; height } : grid) ~(s1 : int) ~(s2 : int)
    : CoordinateSet.t =
  Set.filter coordinates ~f:(fun coordinate ->
      alive_neighbors coordinate { coordinates; width; height }
      |> fun alive_set -> Set.length alive_set = s1 || Set.length alive_set = s2)

(** [spawn_set curr] is the coordinate set of the newly spawned cells *)
let spawn_set ({ coordinates; width; height } : grid) ~(b : int) :
    CoordinateSet.t =
