...
let example_grid = { Base_grid.cells = example_cells; width = 5; height = 5 }
...
let rec input_loop (grid : Base_grid.t) : unit =
  Out_channel.(flush stdout);
  let inp : string option = In_channel.(input_line stdin) in
  let result (grid : Base_grid.t) : Base_grid.t =
    match inp with
    | Some "w" ->
        print_string "going up\n";
        grid
    | Some "n" ->
        let next_grid = Base_grid.next grid in
