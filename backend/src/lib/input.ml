...
let example_cells = Grid.Coordinate_set.of_list [
  { x = 0; y = 1 };
  { x = 0; y = 4 };
  { x = 2; y = 2 };
  { x = 2; y = 3 };
...
let blinker_cells = Grid.Coordinate_set.of_list [
  { x = 2; y = 1 };
  { x = 2; y = 2 };
  { x = 2; y = 3 };
] 
...
let rec input_loop (grid : Grid.t) : unit =
  ...
  let result (grid : Grid.t) : Grid.t = match inp with
  | Some "w" -> print_string "going up\n";  grid
  | Some "n" -> 
    let next_grid = Grid.next grid 
    in
