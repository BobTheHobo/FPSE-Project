...
let coordinate_to_pair_list (ls : Simple_grid.coordinate list) = List.map ls ~f:(fun { x; y } -> (x, y ))
let pair_to_coordinate_list (pairs : (int * int) list) = List.map pairs ~f:(fun (x, y) -> { Simple_grid.x; y })
...
let get_next_obstacles obstacles : Simple_grid.coordinate list =
  ...
  | hd ->
    ...
    let updated = Simple_grid.next { Simple_grid.cells = set; width = 10; height = 10 } in
